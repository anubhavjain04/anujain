...
class NewsController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['News']))
		{
			...
			if($model->newsDate){
				$model->newsDate = date('Y-m-d',strtotime($model->newsDate));
			}else{
				$model->newsDate = null;
			}
			
			if(!$model->newsOrder){
				$model->newsOrder = null;
			}
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['News']))
		{
			...
			if($model->newsDate){
				$model->newsDate = date('m/d/Y',strtotime($model->newsDate));
			}
		}
	...
	{
		$arr=implode(',',$data['news-grid_c0']);
		$connection=Yii::app()->db;
		$query = "update news set status='1' where pkNewsId in(".$arr.")";
	...
	public function multipleUnpublish($data)
	{
	 $connection=Yii::app()->db;
	 $arr=implode(',',$data['news-grid_c0']);
	 $query = "update news set status='0' where pkNewsId in(".$arr.")";
	 $command=$connection->createCommand($query);
	 if($command->execute())
	 {
	  $this->redirect(array('admin'));
	...
	public function multipleDelete($data)
	{
		$connection=Yii::app()->db;
		$arr=implode(',',$data['news-grid_c0']);
		$query = "delete from news where pkNewsId in(".$arr.")";
		$command=$connection->createCommand($query);
		if($command->execute())
		{
	  $this->redirect(array('admin'));
	...
	{
		$connection=Yii::app()->db;
		$query = "update news set status='1' where pkNewsId='".$_GET['id']."'";
		$command=$connection->createCommand($query);
		$command->execute();
		$this->redirect(array('admin'));
	}
	...
	{
		$connection=Yii::app()->db;
		$query = "update news set status='0' where pkNewsId='".$_GET['id']."'";
		$command=$connection->createCommand($query);
		$command->execute();
		$this->redirect(array('admin'));
	}
