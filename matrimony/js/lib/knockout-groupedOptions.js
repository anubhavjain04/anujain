		    ...
		    setSelectedValuesFromSelectNode: function (selectNode, newValue) {
		        ...
		        for (var i = 0; i < nodes.length; i++) {
		            ...
		            if (node.tagName == "OPTION") {
		                setOptionNodeSelectionState(node, ko.utils.arrayIndexOf(newValue, ko.selectExtensions.readValue(node)) >= 0);
		            }
		            else if (node.tagName == "OPTGROUP") {
		                for (var k = 0; k < node.childNodes.length; k++) {
		                    var childNode = node.childNodes[k];
		                    if (childNode.tagName && childNode.tagName == "OPTION") {
		                        setOptionNodeSelectionState(childNode, ko.utils.arrayIndexOf(newValue, ko.selectExtensions.readValue(childNode)) >= 0);
		                    }
		                }
		            }
		        }
