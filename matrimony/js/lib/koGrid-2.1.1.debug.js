...
window.kg.Grid = function (options) {
    ...
    self.setRenderedRows = function (newRows) {
        ...
        if(self.config.selectAllState())
        {
        	//added for remove check sign from multiselect checkbox after delete (refresh)
            self.config.selectAllState(false);
        }
    };
    ...
    self.allSelected.subscribe(function (state) 
    {
        if (self.config.beforeSelectionChange(self.sortedData.peek(), this)) {
            ...
            console.log("About to trigger afterSelectionChange upon SelectALL");
            var selectedRows = [];
           
        	ko.utils.arrayForEach(self.selectionService.rowFactory.rowCache, function(row){
                if (row && row.selected){
                    selectedRows.push(row);
                }
            });
            console.log("Adding rows array with length " + selectedRows.length);
            self.config.afterSelectionChange(selectedRows, this);
