/* jHash v2.0 (http://jhash.codeplex.com) - Copyright (c) 2012 Chris Pietschmann */(function(window){var hashChangeSupported=('onhashchange'in window);var jHash=window.jHash={jhash:"2.0",change:function(handler){if(hashChangeSupported){attachEvent(window,"hashchange",handler);}else{eventHandlers.push(handler);}},unbind:function(handler){var i=0,len=0;if(hashChangeSupported){detachEvent(window,"hashchange",handler);}else{arrayRemove(eventHandlers,eventHandlers.indexOf(handler));}
return this;},val:function(name,value){var ho=jHash.query();if(arguments.length===2){ho[name.toLowerCase()]=(value===null?'':value);return this.set(this.root(),ho);}else if(arguments.length===1&&typeof(name)==='string'){return ho[name.toLowerCase()];}
return ho;},root:function(value){if(value===undefined){return parseHashRoot(window.location.hash);}
return this.set(value,this.val());},set:function(root,query){if(query!==undefined){window.location.hash=root+'?'+(typeof(query)==="string"?query:objectToHash(query));}else{window.location.hash=root;}
return this;},remove:function(name){var ho=jHash.query();ho[name.toLowerCase()]=undefined;return this.set(this.root(),ho);},query:function(){return hashToObject(window.location.hash);},_routes:{},_routeHandlerRegistered:false,_defaultRoute:null,route:function(route,handler){this._routes[route]=handler;if(!this._routeHandlerRegistered){this._routeHandlerRegistered=true;jHash.change(routeHandler);}},processRoute:function(){routeHandler();},defaultRoute:function(root,query){if(arguments.length==0){return this._defaultRoute;}
