...
class MatrimonyMembersController extends Controller
{
	...
	public function actionView($id){
		if($id){
			$data = MatrimonyMembers::model()->getMemberData($id);
	...
	public function actionChangeProfilePic($id)
	{
		...
		if(isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST" and $model)
		{
			...
			if(strlen($name))
			{
				...
				if(in_array($ext,$valid_formats))
				{
					if($size<(1024*1024))
					{
						...
						throw new CHttpException(302,'Image file size max 1 MB');
					}
				}
				else {
					throw new CHttpException(302,'Invalid file format..');
				}
			}

			else {
				throw new CHttpException(302,'Please select image..');
			}
			exit;
		}
	}
	...
	public function actionGetProfilePic($id)
	{
		...
		if($model) {
			...
			throw new CHttpException(404,'This member does not exists.');
		}
	}
						...
						if($size<(1024*1024))
						{
							...
							if(move_uploaded_file($tmp, $path.$actual_image_name))
								...
								//mysql_query("UPDATE users SET profile_image='$actual_image_name' WHERE uid='$session_id'");
									$img["url"] = Yii::app()->baseUrl."/".Yii::app()->params['tempPath']."/".$actual_image_name;
									$this->echoObjectAsJSON($img);
									//echo "<img id='prewImg' src='".Yii::app()->baseUrl."/".Yii::app()->params['tempPath']."/".$actual_image_name."' />";
								}
							else {
								throw new CHttpException(302,'failed to move');
							}
						}
						else {
							throw new CHttpException(302,'Image file size max 1 MB');
