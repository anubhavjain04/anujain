...
class MatrimonyFamilyDetailsController extends Controller
{
	/**
	...
	{
		$model=new MatrimonyFamilyDetails;
		...
		{
			$model->attributes=$_POST['MatrimonyFamilyDetails'];
			if($model->save())
				$this->redirect(array('view','id'=>$model->pkFamilyId));
		}
	...
	public function actionUpdateFamilyDetails($id){
		...
		if($model===null){
			throw new CHttpException(302,'The requested member does not exist.');
			return;
		}
		if($model){
	...
	{
		$dataProvider=new CActiveDataProvider('MatrimonyFamilyDetails');
	...
	{
		$model=new MatrimonyFamilyDetails('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['MatrimonyFamilyDetails']))
			$model->attributes=$_GET['MatrimonyFamilyDetails'];
	...
	{
		$model=MatrimonyFamilyDetails::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='matrimony-family-details-form')
		{
