...
class ProgramsController extends Controller
{
	...
	public function actionCreate()
	{
		...
		{			
			...
			if($model->venueDate){
				$model->venueDate = date('Y-m-d H:i',strtotime($model->venueDate));
			}else{
				$model->venueDate = null;
			}
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Programs']))
		{
			...
			if($model->venueDate){
				$model->venueDate = date('m/d/Y h:i a',strtotime($model->venueDate));
			}
		}
	...
	public function actionLevelRecords(){
		if(isset($_POST['lCode'])){
			...
			if($levelCode=='BRANCH'){
				...
				$criteria1->order = '(ZoneCode*1) ASC';
				$zoneData = $zoneModel->findAll($criteria1);
				foreach($zoneData as $item){
					echo CHtml::tag('option', array('value'=>$item['pkZoneId']),  "Zone-".$item['ZoneCode']);
				}
			}else{
				echo '';
			}
