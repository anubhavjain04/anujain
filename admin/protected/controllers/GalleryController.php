...
class GalleryController extends Controller
{
	...
	{
		$arr=implode(',',$data['gallery-grid_c0']);
		$connection=Yii::app()->db;
		$query = "update gallery set status='1' where pkGalleryId in(".$arr.")";
	...
	public function multipleUnpublish($data)
	{
	 $connection=Yii::app()->db;
	 $arr=implode(',',$data['gallery-grid_c0']);
	 $query = "update gallery set status='0' where pkGalleryId in(".$arr.")";
	 $command=$connection->createCommand($query);
	 if($command->execute())
	 {
	  $this->redirect(array('admin'));
	...
	public function multipleDelete($data)
	{
		$connection=Yii::app()->db;
		$arr=implode(',',$data['gallery-grid_c0']);
		$query = "delete from gallery where pkGalleryId in(".$arr.")";
		$command=$connection->createCommand($query);
		if($command->execute())
		{
	  $this->redirect(array('admin'));
	...
	{
		$connection=Yii::app()->db;
		$query = "update gallery set status='1' where pkGalleryId='".$_GET['id']."'";
		$command=$connection->createCommand($query);
		$command->execute();
		$this->redirect(array('admin'));
	}
	...
	{
		$connection=Yii::app()->db;
		$query = "update gallery set status='0' where pkGalleryId='".$_GET['id']."'";
		$command=$connection->createCommand($query);
		$command->execute();
		$this->redirect(array('admin'));
	}
