...
class MessageController extends Controller
{
	...
	{
		$arr=implode(',',$data['message-grid_c0']);
		$connection=Yii::app()->db;
		$query = "update message set Status='1' where pkMessageId in(".$arr.")";
	...
	public function multipleUnpublish($data)
	{
	 $connection=Yii::app()->db;
	 $arr=implode(',',$data['message-grid_c0']);
	 $query = "update message set Status='0' where pkMessageId in(".$arr.")";
	 $command=$connection->createCommand($query);
	 if($command->execute())
	 {
	  $this->redirect(array('admin'));
	...
	public function multipleDelete($data)
	{
		$connection=Yii::app()->db;
		$arr=implode(',',$data['message-grid_c0']);
		$query = "delete from message where pkMessageId in(".$arr.")";
		$command=$connection->createCommand($query);
		if($command->execute())
		{
	  $this->redirect(array('admin'));
	...
	{
		$connection=Yii::app()->db;
		$query = "update message set Status='1' where pkMessageId='".$_GET['id']."'";
		$command=$connection->createCommand($query);
		$command->execute();
		$this->redirect(array('admin'));
	}
	...
	{
		$connection=Yii::app()->db;
		$query = "update message set Status='0' where pkMessageId='".$_GET['id']."'";
		$command=$connection->createCommand($query);
		$command->execute();
		$this->redirect(array('admin'));
	}
